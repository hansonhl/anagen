...
class AnagenDocument:
    ...
    def decode(self, start, end, in_segment=None, output_str=True):
        ...
        if in_segment and isinstance(in_segment, int):
...
class AnagenDataset(Dataset):
    ...
    def decode(self, doc, start, end, in_segment=None, output="list"):
        if start == -1 and end == -1:
            return "<null>"
        if isinstance(doc, str):
            return self.documents[doc].decode(start, end, in_segment, output)
        if isinstance(doc, AnagenDocument):
            return doc.decode(start, end, in_segment, output)
        else:
            ids = doc[start:end+1]
    ...
    def __getitem__(self, idx):
        ...
        #     print("[CTX]", self.decode_ids(ctx_id))

        # for s, e, anaphor_id in zip(anteced_starts_in_ctx, anteced_ends_in_ctx, anaphor_ids):
            # print("[anteced]", self.decode(doc, s, e), "[anaphor]", self.decode_ids(anaphor_id))

        return doc_key, ctx_set, ctx_starts, ctx_ids, ctx_set_idxs, anteced_starts, anteced_ends, \
            anaphor_starts, anaphor_ids
...
def collate(batch):
    ...
    print("ctx_set_idxs.shape", ctx_set_idxs.shape)
    ...
    print("padded_ctx_ids.shape", padded_ctx_ids.shape)
